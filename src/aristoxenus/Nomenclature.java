...
public class Nomenclature {
    ...
    public static String decodeScientificEnharmonic(String note_name){
        ...
        int sharps = Functions.countOccurrencesInString(note_name, 
                                                    Constants.SHARP_SYMBOL);
        int flats = Functions.countOccurrencesInString(note_name, 
    ...
    public static List<String> bestHeptatonic(String note_name, 
        ...
        for (ScaleSynopsis synopsis: synopseis){
            for (String note: synopsis.scale){
                ...
                if (synopsis.sharps > 0 & synopsis.flats > 0) {synopsis.mixed = true;}
            }
        }
        ...
        if (s_total > f_total){return flat_scale.scale;}
        if (f_total > s_total){return sharp_scale.scale;}

        // Both scales have the same number of accidentals. Return the 
        // pure scale.
        if (sharp_scale.mixed & !flat_scale.mixed){return flat_scale.scale;}
        if (!sharp_scale.mixed & flat_scale.mixed){return sharp_scale.scale;}
